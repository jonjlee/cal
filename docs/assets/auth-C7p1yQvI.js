 ...
 */const $={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},W=r=>(...e)=>({_$litDirective$:r,values:e});class J{constructor(e){}get _$AU(){return this._$AM._$AU}_$AT(e,t,s){this._$Ct=e,this._$AM=t,this._$Ci=s}_$AS(e,t){return this.update(e,t)}update(e,t){return this.render(...t)}}/**
 * @license
 * Copyright 2020 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const Y=r=>r.strings===void 0,G={},Q=(r,e=G)=>r._$AH=e;/**
 * @license
 * Copyright 2020 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const X=W(class extends J{constructor(r){if(super(r),r.type!==$.PROPERTY&&r.type!==$.ATTRIBUTE&&r.type!==$.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!Y(r))throw Error("`live` bindings can only contain a single expression")}render(r){return r}update(r,[e]){if(e===B||e===v)return e;const t=r.element,s=r.name;if(r.type===$.PROPERTY){if(e===t[s])return B}else if(r.type===$.BOOLEAN_ATTRIBUTE){if(!!e===t.hasAttribute(s))return B}else if(r.type===$.ATTRIBUTE&&t.getAttribute(s)===e+"")return B;return Q(r),e}}),Z=0,ee=1,te=2,D=class D extends S{constructor(){super(),this.focusOnUpdate=!1}render(){const e=this.checked==ee,t=this.checked==te,s=this.type=="confirm"?this.type:"",i=this.showHandle?h`<div class="handle">${w("grip-vertical")}</div>`:v,a=this.hideCheckbox?v:h`<input type="checkbox" id="check" .checked=${e} .indeterminate=${t} @change=${this._handleCheckChange}></input>`,n=h`<div
      id="label"
  ...
  `);let q=D;customElements.define("cal-checklist-item",q);class M extends S{constructor(){super(),this.items=[],this.selected=-1,this.deleteIdx=-1}willUpdate(e){this.items=this.items??[],this._renderInfo=this._createRenderInfo(this.items,this.selected,this.deleteIdx),this.items.forEach(t=>delete t.editing)}_createRenderInfo(e,t,s){const i=[];return e.forEach(({id:a,label:n,color:l,checked:u,editing:m},g)=>{i.push({type:"item",id:a,label:n,color:l,checked:u,editing:m,selected:t==g,context:{index:g}})}),s>=0&&s<e.length&&i.splice(s+1,0,{type:"confirm",label:this.deleteWarning,context:{index:s,confirmed:!0}}),{itemInfos:i}}render(){if(!this._renderInfo)return v;const e=this._renderInfo.itemInfos.map((s,i)=>h`<cal-checklist-item
        type=${s.type}
        label=${s.label}
        color=${s.color}
        checked=${s.checked}
